...
if(isset($_POST["category"])){
	...
	echo "
		<div class='nav nav-pills nav-stacked'>
			<li class='active'><a href='#'><h4>Categorii de produse</h4></a></li>
	";
...
if(isset($_POST["brand"])){
	...
	echo "
		<div class='nav nav-pills nav-stacked'>
			<li class='active'><a href='#'><h4>Brand-uri</h4></a></li>
	";
	if(mysqli_num_rows($run_query) > 0){
		while($row = mysqli_fetch_array($run_query)){
			$bid = $row["brand_id"];
			...
			echo "
				<div class='col-md-4'>
							<div class='panel panel-info'>
								...
								<div class='panel-heading'>$pro_price Lei
									<button pid='$pro_id' style='float:right;' id='product' class='btn btn-danger btn-xs'>Adauga in cos</button>
								</div>
							</div>
						</div>	
			";
		...
		if($count > 0){
			echo "
				<div class='alert alert-warning'>
						...
						<b>Produsul se afla deja in cosul dvs. de cumparaturi!</b>
				</div>
			";//not in video
		} else {
			...
			if(mysqli_query($con,$sql)){
				echo "
					<div class='alert alert-success'>
						<a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>
						<b>Produsul a fost adaugat in cosul dvs. de cumparaturi!</b>
					</div>
				";
			}
		}
			...
			while ($row=mysqli_fetch_array($query)) {
				...
				echo '
					<div class="row">
						...
						<div class="col-md-3">'.$product_price.' Lei</div>
					</div>';
				
			}
			?>
				<a style="float:right;" href="cart.php" class="btn btn-warning">Editeaza&nbsp;&nbsp;<span class="glyphicon glyphicon-edit"></span></a>
			<?php
			exit();
		}
	}
				...
				if (!isset($_SESSION["uid"])) {
					...
					echo '
						...
						<form action="https://www.sandbox.paypal.com/cgi-bin/webscr" method="post">
							...
							<input type="hidden" name="business" value="xpharmonline@gmail.com">
...
if (isset($_POST["removeItemFromCart"])) {
	...
	if(mysqli_query($con,$sql)){
		echo "<div class='alert alert-danger'>
						<a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>
						<b>Produsul a fost eliminat din cosul dvs. de cumparaturi!</b>
				</div>";
		exit();
	}
}
...
if (isset($_POST["updateCartItem"])) {
	...
	if(mysqli_query($con,$sql)){
		echo "<div class='alert alert-info'>
						<a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>
						<b>Cosul dvs. de cumparaturi a fost actualizat!</b>
				</div>";
		exit();
	}
}
